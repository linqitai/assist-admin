		...
		<div class="container">
			...
			<el-table :data="tableData" border stripe class="table" ref="multipleTable" style="width: 100%">
					...
					<template slot-scope="props">
						<div class="flexBox">
							<div class="flexBoxLeft" v-if="props.row.pic">
								<el-image style="width: 60px; height: 80px" v-for="(item,index) in props.row.headPic.split('|')" :src="item"
								 :key="index" :preview-src-list="props.row.headPic.split('|')">
								</el-image>
								<el-image style="width: 60px; height: 80px" v-for="(item,index) in props.row.pic.split('|')" :src="item" :key="index"
				...
				<el-table-column label="是否显示" width="120" fixed="right">
					<template slot-scope="scope">
						<el-switch v-model="scope.row.isShow" active-color="#ffae00" inactive-color="#c7c7c7" active-text="是"
						 inactive-text="否" @change="isShowChange(scope.row)">
						</el-switch>
					</template>
				</el-table-column>
		...
		<el-dialog :title="visibleType=='detail'?'详情':'编辑'" :visible.sync="detailOrEditVisible" fullscreen>
			<el-form ref="form" :model="form" label-width="auto" :inline="true" :disabled="visibleType=='detail'?true:false">
				...
				<el-form-item label="故事">
					<el-input v-model="form.story" class="width800" type="textarea" :autosize="{ minRows: 6, maxRows: 16}">
					</el-input>
				</el-form-item>
				...
				<el-form-item label="驳回原因">
					<el-input v-model="form.remark" class="width800" type="textarea" :autosize="{ minRows: 1, maxRows: 3}">
					</el-input>
				</el-form-item>
			</el-form>
